...
| Android runtime | C | New in this fork: host-side Go ADB bridge in `apps/OpenAndroidUse` exposing the same 9 tools, MCP server, and `call --calls`; uiautomator snapshot + screencap with the macOS image budget model, single CoordinateScale invariant pinned by unit tests; mocked-adb test suite passes in CI containers. Companion mode (`OPEN_ANDROID_USE_COMPANION=1`) is companion-first end to end — snapshots, gestures, Unicode text — with ADB fallback, pinned by httptest-backed unit tests. Not yet validated on real hardware; no smoke fixture or npm distribution. | Run the `VERIFICATION.md` checklist on hardware, emulator-based smoke runner, npm bundling. |

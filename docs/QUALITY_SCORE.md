...
| Android runtime | C | New in this fork: host-side Go ADB bridge in `apps/OpenAndroidUse` exposing the same 9 tools, MCP server, and `call --calls`; uiautomator snapshot + screencap with the macOS image budget model, single CoordinateScale invariant pinned by unit tests; mocked-adb test suite passes in CI containers. Not yet validated on real hardware; ASCII-only typing over ADB; no smoke fixture or npm distribution. | Device smoke on real hardware, emulator-based smoke runner, npm bundling, then the Phase 2 on-device companion (AccessibilityService + IME) per `docs/exec-plans/active/20260612-android-use-runtime.md`. |

...
| On-device agent (3.1a) | C | Companion-hosted agent loop on the official Anthropic Java SDK (pinned 2.40.1): chat UI, streaming with adaptive thinking, frozen 9-tool schema executed in-process with bridge-identical semantics, prompt-cache breakpoint, refusal handling, stop gate, Keystore-held API key, screenshot pruning. Builds and unit-tests green in container; zero on-device runs yet — V33–V37 in `VERIFICATION.md` own that. | Hardware verification (V33–V37), 3.1b safety surfaces (touch-to-pause, confirmation sheet, gesture trail), models-API-driven model list. |

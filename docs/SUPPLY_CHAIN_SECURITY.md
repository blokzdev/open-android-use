...
| `com.google.ai.edge.litertlm:litertlm-android` 0.13.1 (Google Maven) | Apache-2.0 | `apps/OpenAndroidUseCompanion` `agent/llm` only | First-party LiteRT-LM runtime for on-device Gemma 4 E2B inference (Phase 5.5b). Ships native `.so` libraries (`arm64-v8a` + `x86_64` via `abiFilters`; 32-bit excluded); the model is downloaded at runtime, never bundled. Required the project Kotlin bump to 2.3.20 (its metadata is Kotlin 2.3) + AGP 8.13.2. | 2026-06-15 |
